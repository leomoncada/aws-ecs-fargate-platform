...
- **Health checks:** Liveness and readiness are separate endpoints, and only liveness is wired to the ALB target group. Backend `/health` and frontend `/api/health` answer for their own process. Frontend `/api/health/ready` also probes the backend and returns 503 when it is unreachable, but nothing routes on it. Checking a downstream from the endpoint the load balancer polls turns a backend blip into a full outage: every frontend task leaves the target group at once and the ALB has no healthy target left.
